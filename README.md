...
| [uniswap-lp](python/uniswap-lp/)                       | ![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white)             | **Provide liquidity on Uniswap V3.** Mints a USDC/WETH LP position — the vault holds the NFT. Shows the multi-token approval pattern: persistent WETH approval + auto USDC approval in one flow.                                    |
| [uniswap-lp](typescript/uniswap-lp/)                   | ![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white) | **Provide liquidity on Uniswap V3.** Same LP flow in TypeScript — mint, remove, collect. Demonstrates `amount=0` for persistent approvals and no-approval protocol calls.                                                           |
| [scheduled-payments](python/scheduled-payments/)       | ![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white)             | **Recurring payments on autopilot.** Define a schedule in JSON (who, how much, how often) and the bot sends payments automatically. Payroll, subscriptions, API credits — with spending limits enforced by the vault.               |
...
| [batch-payments](typescript/batch-payments/)           | ![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white) | **Pay a list of people at once.** Load a JSON or CSV of recipients and amounts, bot pays them sequentially. Bounty payouts, revenue splits, airdrops — with spending limits on each.                                                |
...
| [vault-rebalance](python/vault-rebalance/)             | ![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white)             | **Swap tokens inside the vault.** Demonstrates `executeSwap()` with bot-signed `fromToken` and `maxFromAmount` — the bot controls which token is sold and caps the cost. Output stays in the vault.                                 |
| [vault-rebalance](typescript/vault-rebalance/)         | ![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white) | **Swap tokens inside the vault.** Same rebalancing flow in TypeScript. Bot signs both sides of the swap — relayer cannot substitute the input token or overspend.                                                                    |
